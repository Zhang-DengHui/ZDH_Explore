PokerTheTaskPop3Panel = {}
local this = PokerTheTaskPop3Panel
PObject.extend(this)
...
function this.initLayer()
	Log.i("PokerTheTaskPop3Panel initLayer")
	...
	this.widgetTable.Button_ljpm:loadTextures(imagePath .. "nav_bg_select_02.png", "", imagePath .. "nav_bg_select_02.png")
...
function this.removeLayer()
	Log.i("PokerTheTaskPop3Panel removeLayer")
...
function this.updateWithShowData(showdata)
	Log.i("PokerTheTaskPop3Panel updateWithShowData")
	if not this.mainLayer then
		Log.w("PokerTheTaskPop3Panel mainLayer is not ready")
		return
	end
	if not showdata then
		Log.w("PokerTheTaskPop3Panel showdata is not ready")
		return
	else
		this.dataTable.showData = showdata
		PLTable.print(showdata,"PokerTheTaskPop3Panel showdata")
	end

	if showdata.total then
		local tmpCount = #showdata.total
		for i=1,tmpCount do
			this.widgetTable["Label_name" .. i]:setText(PokerTheTaskDetailPanel.zodiacTable[showdata.total[i]["zodiac"]])
			this.widgetTable["Label_score" .. i]:setText(showdata.total[i]["score"])
			this.widgetTable["Label_num" .. i]:setText(showdata.total[i]["count"])
			this.widgetTable["Label_change" .. i]:setText(showdata.total[i].trend)
...
function this.show()
	Log.i("PokerTheTaskPop3Panel show")
	if this.mainLayer then
		return
	end
	if not this.mainLayer then
