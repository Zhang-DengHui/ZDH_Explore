...
function this.initLayer()
	Log.i("PokerRecallPanel initLayer")
	...
	if not this.widgetTable.mainWidget then
		Log.w("PokerRecallPanel Read WidgetFromJsonFile Fail")
		return
	end

	if this.mainLayer then
		Log.w("PokerRecallPanel mainLayer is in dont need create new layer")
		return
	end
	...
	if this.widgetTable.Button_close ~= nil then
		local function clickclose( sender, eventType)
			if eventType == 2 then
         		Log.d("PokerRecallPanel click_close")
	...
	this.widgetTable.Image_title:loadTexture(imagePath .. "title.png")
	...
	this.widgetTable.Image_back_bg:loadTexture(imagePath .. "bg_top.png")
	...
	if this.widgetTable.Button_wen then
		local function clickshowinfo( sender, eventType)
			if eventType == 2 then
         		Log.d("PokerRecallPanel click_showinfo")
         		this.sendMsgToCtrl(8)
         		PokerRecallInfoPanel.show()	
      		end
		end
	...
	this.widgetTable.ProgressBar_Left:loadTexture(imagePath .. "prog_green.png")
	...
	this.widgetTable.ProgressBar_right:loadTexture(imagePath .. "prog_green.png")
...
function this.removeLayer()
	Log.i("PokerRecallPanel removeLayer")
...
function this.updateShowdata( showdata ,isoffset)
	...
	--设置时间
  	local actTimeString = string.format([[活动时间:%s-%s]], os.date("%m月%d日",tonumber(this.dataTable.showData.ams_resp.beginTime)),os.date("%m月%d日",tonumber(this.dataTable.showData.ams_resp.EndTime))) 
  	this.widgetTable.Label_time:setText(actTimeString)

  	--左侧数据更新
  	local leftnumString = string.format([[召唤（%s/%s）个好友]],this.dataTable.showData.ams_resp.total_RecallNum,this.dataTable.showData.ams_resp.totoal_InvitePacagesNum) 
  	...
  	if this.dataTable.showData.ams_resp.total_RecallNum >= this.dataTable.showData.ams_resp.totoal_InvitePacagesNum and tostring(this.dataTable.showData.ams_resp.gettotalgift5_is_used) ~= "1" then
  		this.widgetTable.Button_bag_left:setTouchEnabled(true)
  		local function clicklq ( sender, eventType)
			if eventType == 2 then
         		Log.d("PokerRecallPanel click_lqzh")
		...
		this.widgetTable.Button_bag_left:addTouchEventListener(clicklq)
  		...
  		this.widgetTable.ProgressBar_Left:setVisible(true)
  		this.widgetTable.ProgressBar_Left:setPercent(12+88/this.dataTable.showData.ams_resp.totoal_InvitePacagesNum*this.dataTable.showData.ams_resp.total_RecallNum)
  	end
  	if this.dataTable.showData.ams_resp.total_RecallNum >= this.dataTable.showData.ams_resp.totoal_InvitePacagesNum then
  		this.widgetTable.ProgressBar_Left:setVisible(true)
  		this.widgetTable.ProgressBar_Left:setPercent(100)
  	end
  	if tonumber(this.dataTable.showData.ams_resp.total_RecallNum) == 0 then
  		this.widgetTable.ProgressBar_Left:setVisible(false)
  	end

  	--右侧数据更新
  	local rightnumString = string.format([[成功召唤（%s/%s）个好友]],this.dataTable.showData.ams_resp.total_RecallSuccNum,this.dataTable.showData.ams_resp.totoal_SuccInvitePacagesNum) 
  	...
  	if this.dataTable.showData.ams_resp.total_RecallSuccNum >= this.dataTable.showData.ams_resp.totoal_SuccInvitePacagesNum and tostring(this.dataTable.showData.ams_resp.gettotalsuccgift6_is_used) ~= "1" then
  		this.widgetTable.Button_bag_right:setTouchEnabled(true)
  		local function clicklq ( sender, eventType)
			if eventType == 2 then
         		Log.d("PokerRecallPanel click_lqczh")
		...
		this.widgetTable.Button_bag_right:addTouchEventListener(clicklq)
  		...
  		this.widgetTable.ProgressBar_right:setVisible(true)
  		this.widgetTable.ProgressBar_right:setPercent(12+88/this.dataTable.showData.ams_resp.totoal_SuccInvitePacagesNum*this.dataTable.showData.ams_resp.total_RecallSuccNum)
  	end
  	if this.dataTable.showData.ams_resp.total_RecallSuccNum >= this.dataTable.showData.ams_resp.totoal_SuccInvitePacagesNum then
  		this.widgetTable.ProgressBar_right:setVisible(true)
  		this.widgetTable.ProgressBar_right:setPercent(100)
  	end
  	if tonumber(this.dataTable.showData.ams_resp.total_RecallSuccNum) == 0 then
...
function this.scrollviewToTableview(rootView)
	Log.d("PokerRecallPanel scrollviewToTableview")
...
function this.tableCellAtIndex( table, index )
	Log.d("PokerRecallPanel index "..index)
...
function this.updateItem(idx,itemtype,dataidx,isremove,parentCell,isleft)
	-- Log.i("PokerRecallPanel updateItem idx "..idx.." ,itemtype "..itemtype.." ,dataidx "..dataidx.." ,isremove "..tostring(isremove))
	if not parentCell then
		Log.w("PokerRecallPanel parentCell is nil")
		return
	end
	...
	if isremove then
		...
		if itemCellWidget then
			Log.i("PokerRecallPanel itemCellWidget remove idxis "..idx)
			itemCellWidget:removeFromParentAndCleanup(true)
		else
			Log.i("PokerRecallPanel itemCellWidget is already nil")
		end
	...
	if not itemCellWidget then
    	local itemWidget = GUIReader:shareReader():widgetFromJsonFile("json/PokerRecall/PokerRecallPanelData.json")
    	if not itemWidget then
			Log.w("PokerRecallPanelData Read WidgetFromJsonFile Fail")
			return
		end
  	...
  	if this.dataTable.itemdata then
    	if this.dataTable.itemdata[itemtype] then
    		if this.dataTable.itemdata[itemtype][dataidx] then
    			if this.dataTable.itemdata[itemtype][dataidx].sFriendUin then
	...
	Image_white:loadTexture(imagePath .. "cgzh.png")
	...
	if itemtype == 1 then
		...
		local function clicklq ( sender, eventType)
			if eventType == 2 then
         		Log.d("PokerRecallPanel click_lq")
         		this.sendMsgToCtrl(3,this.dataTable.showData.ams_resp.friendsCanGetGiftInfo[dataidx])
      		end
		end
			...
			for i=1,#this.dataTable.showData.ams_resp.showInvitedPacages do
				if i == 1 then
  				...
  				elseif i == 2 then
  					...
  					Log.w("PokerRecallPanelData InvitingPacages num is out")
				end
			end
  		end
	elseif itemtype == 2 then
		--可召回
		if iInviteType == "0" then
			Button_state:loadTextureNormal(imagePath .. "btn_zh.png")
			local function clicklq ( sender, eventType)
				if eventType == 2 then
	         		Log.d("PokerRecallPanel click_zh")
	         		this.sendMsgToCtrl(2,this.dataTable.showData.ams_resp.friendsCanRevInfo[dataidx])
	      		end
			end
			Button_state:addTouchEventListener(clicklq)
			Button_state:setTouchEnabled(true)
		...
		--再次召回
			...
			Button_state:loadTextureNormal(imagePath .. "btn_zczh.png")
			local function clicklq ( sender, eventType)
				if eventType == 2 then
	         		Log.d("PokerRecallPanel click_zh")
	         		this.sendMsgToCtrl(2,this.dataTable.showData.ams_resp.friendsCanRevInfo[dataidx])
	...
	elseif itemtype == 3 then
		--召回中
		Button_state:loadTextureNormal(imagePath .. "btn_zhz.png")
	...
	elseif itemtype == 4 then
		--已领取
		Button_state:loadTextureNormal(imagePath .. "btn_ylj.png")
		Button_state:setTouchEnabled(false)
	...
	if this.dataTable.shortPlayerList then
		if not this.dataTable.shortPlayerList[tostring(itemCellWidget.uid)] then
			Log.w("PokerRecallPanel itemCellWidget idx " ..idx.. " uid "..itemCellWidget.uid.." in this.dataTable.shortPlayerList is nil")
...
function this.imageHeadToSprit(rootImageView,path)
	...
	local create_head_gif = function(rootImageView, path)
		local sprite = GifWrapper.create({ path = path, 0, 0 })
		if not sprite then
...
function this.sendMsgToCtrl(msgtype,msgdata,msgflag)
	Log.i("PokerRecallPanel.sendMsgToCtrl msgtype == "..msgtype)
	PokerRecallCtrl.getMsgFromPanel(msgtype,msgdata,msgflag)
end

function this.getMsgFromCtrl(msgtype,msgdata,msgflag)
	Log.i("PokerRecallPanel.getMsgFromCtrl msgtype == "..msgtype)
    if msgtype == 1 then
        this.initLayer()
    ...
    elseif msgtype == 5 then
        ...
        Log.w("PokerRecallPanel.getMsgFromPanel msgtype is out" )
    end
end
